...
class Template
{
	...
	private function _ext($file)
	{
        ...
        function load($view = '' , $view_data = array(), $return = FALSE)	{               
			$output = $this->_ci->load->view($view, $view_data, TRUE);	

			if(preg_match('/<!-- #[\r\n]*(.*?)[\r\n]*-->/s', $output, $matches)) {
				# 	$matches[1] contains:
				#
				#		$this->setTitle("Welcome to CodeIgniter");
    			#		$this->setLayout(null);
				#	
			...
			if($this->layout) {
				$this->set1("contents", $output);
				$output = $this->_ci->load->view($this->layout, $this->template_data, TRUE);
			}
				#	auto parse modules if ready exist.
			$output = @preg_replace('/<!-- #MOD (.*?)\((.*?)\) -->/se', '$this->CI->modules->\\1("\\2");', $output);
			
			if($return) return $output;
			echo $output;
		}	
        	function ajax_load_view($view = '' , $view_data = array(), $return = FALSE)	{               
			$output = $this->_ci->load->view($view, $view_data, TRUE);	
